...
from biosteam._graphics import vertical_column_graphics
from biosteam import Unit
from biosteam import design_tools
from biosteam.units import FakeSplitter
...
from biosteam.units.heat_exchange import HXutility
import numpy as np
...
class PlasmaReactor(bst.Unit):
    ...
    def __init__(
        ...
        # yields is a dictionary of the form {product: yield} like {"CO2": 0.5, "H2O": 0.5}
    ...
    def _run(self):
        i = self.ins[0] #input stream
        o = self.outs[0] #output streams
        o.copy_like(i) # copy_like: outputs have same conditions or states as inputs

        # Plasma reaction Gabbar et al. 2017
        # out = rxplasma.outs[0]
        # Paraffins 	10.18
        # Olefins	11.79
        # Alcohol	52.03
        # Carboxylic Acid	12.33
        # Other Carbonyl	7.69
        # Unquantified in GC (C29-C44)	15.90

        for product, y in self.yields.items():
            o.set_flow(i.get_flow("kg/hr", "Plastic") * y, "kg/hr", product)  #setting flow rate for products in yield list 

        for species in [i.ID for i in i.available_chemicals]:
            if species not in self.yields:
                input = i.get_flow("kg/hr", species)
                # output = max([0, input * (1 - sum(self.yields.values()))])
                output = input
                o.set_flow(output, "kg/hr", species)
        
        o.imass["CO2"] = max([0, i.imass["CO2"] - i.imass["Plastic"]*0.1]) # The CO2 added to the plastic is removed from the CO2 stream
        # o.imass["O2"] = max([0, i.imass["O2"] - i.imass["Plastic"]*0.1]) # The O2 added to the plastic is removed from the O2 stream
        o.imass["O2"] = 0 # The O2 is consumed in the reactor
        o.imass["Plastic"] = 0.001*i.imass["Plastic"] # 0.1% of the plastic is converted to gas
        o.T = 273.15+400 # set temperature of the output at 400 celcius, vapor products 
        

    def _design(self):
        i = self.ins[0]
        # power_input = i.get_total_flow("tonne/hr") * self.power # plasma power input 
        # self.add_power_utility(power_input)
        self.add_power_utility(self.power)

        # Dave and Joshi "Plasma pyrolysis and gasification of plastic waste - A review"
        # rxplasma.add_power_utility(
        #     plasmaEnergy * feed.get_total_flow("tonne/hour")  # used to be 0.25
        # )  # Double-check power utility calculations
...
class Crystalizer(bst.Unit):
    _N_ins = 1  # number of inlet
    _N_outs = 1  # Number of outlets

    # _units =
    def __init__(self, ID, ins=(), outs=(), thermo=None, Tin=32 + 273.15, time=1):
        bst.Unit.__init__(self, ID, ins, outs, bst.settings.get_thermo())
        self.time = time
        self.Tin = Tin
        #  self.ins = ins
    ...
    def _run(self):
        input = self.ins[0]
        Yield_factor = 0.9
        growth_rate = 5  # kg/m3hr # Crystal growth rate
        Volume = input.get_total_flow("kg/hr") / (growth_rate * Yield_factor)
        self.time = (Volume / input.get_total_flow("kg/hr")) * 1000
        self.cystals = input.get_total_flow("kg/hr") * 0.9
        self.liq = input.get_total_flow("kg/hr") * 0.1

    def _design(self):
        cooling_rate = 5  # C/min
        dT = cooling_rate * self.time
        T_out = self.Tin - dT
        if T_out < 275:
            print("Crystallizer temperature below 275")
            T_out = 275
        cool_utility = self.add_heat_utility(dT, T_out)
        self.outs[0].T = T_out
        # change time to hour,
    ...
    #    def _cost(self):
    def _cost(self):
        self.baseline_purchase_costs["Crystalizer"] = (
            841000 * (self.ins[0].get_total_flow("kg/hr") / (3.58 * 1000)) ** 0.7
        )


class Centrifuge(bst.units.SolidsCentrifuge):
    _F_BM_default = {"Centrifuge": 2.1}

    def _cost(self):
        self.baseline_purchase_costs["Centrifuge"] = (
            533080 * (self.ins[0].get_total_flow("kg/hr") / (33.17 * 1000)) ** 0.7
        )  # Kwan Hu Lin 2018

    # self.baseline_purchase_costs['Centrifuge'] = 199548*(self.ins[0].get_flow("tonnes/year", "Water")/350)**0.7 # Edlund et al 2016


class Filtration(bst.units.Splitter):
    _F_BM_default = {"Filters": 2.1}

    def _cost(self):
        self.baseline_purchase_costs["Filters"] = (
            3910000 * (self.ins[0].get_total_flow("kg/hr") / (37.61 * 1000)) ** 0.7
        )  # Kwan Hu Lin 2018

class AutoShortcutColumn(bst.units.ShortcutColumn):
    def __init__(
        self,
        ID="",
        ins=None,
...
class MyDistillation(Unit):
    ...
    def _run(self):
        ...
        # backup option if the run fails
        if self.Lr is not None:

            def split_input_top(x):
                self.outs[0].imass[x.ID] = self.ins[0].imass[x.ID] / 2

            map(split_input_top, self.ins[0].available_chemicals)
            self.outs[0].imass[self.LHK[0]] = self.ins[0].imass[self.LHK[0]] * self.Lr

            def split_input_bot(x):
                self.outs[1].imass[x.ID] = self.ins[0].imass[x.ID] / 2

            map(split_input_bot, self.ins[0].available_chemicals)
            self.outs[1].imass[self.LHK[1]] = self.ins[0].imass[self.LHK[1]] * self.Hr
        else:
            ...
            map(split_input_top, self.ins[0].available_chemicals)
            self.outs[0].imass[self.LHK[0]] = (
                self.ins[0].imass[self.LHK[0]] + self.ins[0].imass[self.LHK[1]]
            ) * self.y_top
    ...
    def _load_components(self, partial_condenser, condenser_thermo, boiler_thermo, LHK):
        ...
        #: [HXutility] Condenser.
        if not condenser_thermo:
            condenser_thermo = thermo
        ...
        #: [HXutility] Boiler.
        if not boiler_thermo:
            boiler_thermo = thermo
        self.boiler = HXutility(
            None,
    ...
    def _update_distillate_and_bottoms_temperature(self):
        ...
        condenser_distillate.copy_flow(distillate)
        ...
        self.condenser.T = (
            self.condensate.T
        ) = condenser_distillate.T = distillate.T = p.T
        self.condenser.P = (
            self.condensate.P
        ) = condenser_distillate.P = distillate.P = p.P
    ...
    def product_specification_format(self, spec):
        if spec == "Composition":
            ...
            raise AttributeError(
                "product specification format must be either "
    ...
    def y_top(self, y_top):
        ...
        assert (
            0 < y_top < 1
        ), "light key composition in the distillate must be a fraction"
        self._y_top = y_top
    ...
    def x_bot(self, x_bot):
        ...
        assert (
            0 < x_bot < 1
        ), "heavy key composition in the bottoms product must be a fraction"
        self._x_bot = x_bot
    ...
    def tray_type(self, tray_type):
        if tray_type in distillation_tray_type_factor:
            ...
            F_D = self.F_D
            F_D["Trays"] = F_D["Stripper trays"] = F_D[
                "Rectifier trays"
            ] = distillation_tray_type_factor[tray_type]
        else:
            raise ValueError(
                "tray type must be one of the following: "
    ...
    def tray_material(self, tray_material):
        if tray_material in tray_material_factor_functions:
            ...
            raise ValueError(
                "tray material must be one of the following: "
    ...
    def vessel_material(self, vessel_material):
        if vessel_material in distillation_column_material_factors:
            ...
            F_M = self.F_M
            F_M["Rectifier tower"] = F_M["Stripper tower"] = F_M[
                "Tower"
            ] = distillation_column_material_factors[vessel_material]
        else:
            raise ValueError(
                "vessel material must be one of the following: "
    ...
    def is_divided(self, is_divided):
        ...
        self.line = (
            "Divided Distillation Column" if is_divided else "Distillation Column"
        )

    def _set_distillation_product_specifications(
        self, product_specification_format, x_bot, y_top, Lr, Hr
    ):
    ...
    def _check_mass_balance(self):
        ...
        if self.product_specification_format == "Composition":
            if LK_distillate < 0.0 or LK_bottoms < 0.0:
                raise InfeasibleRegion(
                    region="light key molar fraction",
                    msg=(
                        "the molar fraction of the light key in the feed must be "
                        "between the bottoms product and distillate compositions "
                        "(i.e. z_bottoms_LK < z_feed_LK < z_distillate_LK)"
                    ),
                )
            if HK_distillate < 0.0 or HK_bottoms < 0.0:
                raise InfeasibleRegion(
                    region="heavy key molar fraction",
                    msg=(
                        "the molar fraction of the heavy key in the feed must be "
    ...
    def _run_binary_distillation_mass_balance(self):
        ...
        distillate.mol[intemediates_index] = bottoms_product.mol[intemediates_index] = (
            mol[intemediates_index] / 2
        )
    ...
    def _setup(self):
        super()._setup()
        distillate, bottoms_product = self.outs
        self.boiler.ins[0].P = self.condenser.ins[0].P = self.condenser.outs[
            0
        ].P = self.mixed_feed.P = distillate.P = bottoms_product.P = self.P
    ...
    def _run_condenser_and_boiler(self):
        ...
        vap.T = vap_T
        vap.P = distillate.P
        if not self._partial_condenser:
            self.splitter.ins[0].mix_from(self.splitter.outs)
    ...
    def _compute_N_stages(self):
        ...
        if E_eff:
            ...
            (
                ...
                alpha_LHK_bottoms,
            ) = self._get_relative_volatilities_LHK()
            F_mol_distillate = self.F_Mol_distillate
            L_Rmol = self.F_Mol_condensate
            V_Rmol = (R + 1) * F_mol_distillate
            E_rectifier = design.compute_murphree_stage_efficiency(
                mu, alpha_LHK_distillate, L_Rmol, V_Rmol
            )
            ...
            E_stripper = design.compute_murphree_stage_efficiency(
    ...
    def _complete_distillation_column_design(self):
        ...
        if Po < 14.68:
            warn(
                "vacuum pressure vessel ASME codes not implemented yet; "
        ...
        if is_divided:
            ...
            Design["Rectifier height"] = H_R = (
                design.compute_tower_height(TS, Rstages - 1) * 3.28
            )
            Design["Stripper height"] = H_S = (
                design.compute_tower_height(TS, Sstages - 1) * 3.28
            )
            Design["Rectifier diameter"] = R_diameter
            Design["Stripper diameter"] = S_diameter
            Design[
                "Rectifier wall thickness"
            ] = tv = design.compute_tower_wall_thickness(Po, R_diameter, H_R)
            Design[
                "Stripper wall thickness"
            ] = tv = design.compute_tower_wall_thickness(Po, S_diameter, H_S)
            Design["Rectifier weight"] = design.compute_tower_weight(
                R_diameter, H_R, tv, rho_M
            )
            Design["Stripper weight"] = design.compute_tower_weight(
                S_diameter, H_S, tv, rho_M
            )
        else:
            Design["Actual stages"] = Rstages + Sstages
            Design["Height"] = H = (
                design.compute_tower_height(TS, Rstages + Sstages - 2) * 3.28
            )
            Design["Diameter"] = Di = max((R_diameter, S_diameter))
            Design["Wall thickness"] = tv = design.compute_tower_wall_thickness(
                Po, Di, H
            )
    ...
    def _cost(self):
        ...
        if self.is_divided:
            ...
            N_RT = Design["Rectifier stages"] - 1.0
            ...
            N_ST = Design["Stripper stages"] - 1.0
            ...
            Cost["Stripper platform and ladders"] = design.compute_plaform_ladder_cost(
                Di_R, H_R
            )
            W_S = Design["Stripper weight"]  # in lb
            H_S = Design["Stripper height"]  # in ft
            Cost["Stripper tower"] = design.compute_empty_tower_cost(W_S)
            Cost["Rectifier platform and ladders"] = design.compute_plaform_ladder_cost(
                Di_S, H_S
            )
            ...
            N_T = Design.get("Actual stages", 13) - 1.0
            Di = Design.get("Diameter", 3)
            F_M["Trays"] = self._F_TM_function(Di)
            Cost["Trays"] = design.compute_purchase_cost_of_trays(N_T, Di)

            # Cost vessel assuming T < 800 F
            W = Design.get("Weight", 2000)  # in lb
            H = Design.get("Height", 15)  # in ft
...
class MyBinaryDistillation(bst.units.Distillation, new_graphics=False):
    r"""
    ...
    """
    _cache_tolerance = np.array([50.0, 1e-5, 1e-6, 1e-6, 1e-2, 1e-6], float)
    ...
    def _run_McCabeThiele(self):
        ...
        if (abs(old_args - args) < self._cache_tolerance).all():
            return
        ...
        Rmin_intersection = (
            lambda x: q_line(x) - solve_Ty(np.array((x, 1 - x)), P)[1][0]
        )
        ...
        compute_stages_McCabeThiele(
            P, rs, x_stages, y_stages, T_stages, y_top, solve_Ty
        )
    ...
    def _plot_stages(self):
        ...
        if not hasattr(self, "_x_stages"):
            raise RuntimeError(
                "cannot plot stages without running McCabe Thiele binary distillation"
            )
        x_stages = self._x_stages
        y_stages = self._y_stages
        LHK = self.LHK
    ...
    def plot_stages(self):
        ...
        plt.legend(
            [
                f"Stages: {stages}, Feed: {feed_stage}",
                "Graphical aid",
                "eq-line",
                "q-line",
                "ROL",
