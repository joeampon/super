...
cpy    = {"Technology":"CPY","Hydrocracking": "No", "Yield": cpy_comp_yield,"Reactor size" : 1,"wt_closure": 92.5, "NG_req":2.0,"residence_time" :"low"   , "name" : "cpy"}
cpy_hc = {"Technology":"CPY","Hydrocracking": "Yes", "Yield": cpy_comp_yield,"Reactor size" : 1,"wt_closure": 92.5, "NG_req":2.0,"residence_time" : "low" , "name" : "cpy_hc"}
tod_hc = {"Technology":"TOD","Hydrocracking": "Yes", "Yield": tod_comp_yield,"Reactor size" : 0.4,"wt_closure":90.4, "NG_req":0.0,"residence_time" :"low" , "name" : "tod"}
tod    = {"Technology":"TOD","Hydrocracking": "No", "Yield": tod_comp_yield, "Reactor size" : 0.4,"wt_closure":90.4, "NG_req":0.0,"residence_time" :"low" , "name" : "tod_hc"}
hrt    = {"Technology":"CPY","Hydrocracking": "No", "Yield": hrt_comp_yield, "Reactor size" : 1, "wt_closure": 92.5, "NG_req":2.0,"residence_time" :"high", "name" : "hrt"}
hrt_hc = {"Technology":"CPY","Hydrocracking": "Yes", "Yield": hrt_comp_yield,"Reactor size" : 1,"wt_closure": 92.5, "NG_req":2.0,"residence_time" :"high" , "name" : "hrt_hc"}

scenarios = [cpy,cpy_hc,tod,tod_hc,hrt,hrt_hc]
scenarios_labels = ["CPY","CPY-FCC","TOD","TOD-FCC","HRT","HRT-FCC"]
    ...
    # Power in KW/hr for hydroprocessing 1811, hydrocracking electrucak cpnsumption 369kw, recycle compressor = 31
        ...
        with bst.System('sys_cat_cracking') as sys_cat_cracking:           
            ...
            catalytic_cracking_reaction = tmo.ParallelReaction([
                ...
                tmo.Reaction('C24H50 -> C12H26 + C12H24',  reactant='C24H50',  X=0.7, correct_mass_balance=True),
                ])
            if scenario['residence_time'] == "low":
                hydro_crack = FluidizedCatalyticCracking("Cat_cracking",ins=(M4-0,cracking_catalyst,air,steam),reaction=catalytic_cracking_reaction)
...
# Consolidate product streams
    ...
    if scenario['Hydrocracking'] == "Yes":
        ...
        cat_crack.purchase_costs = {"Hydrocrackng_Unit_mix":0} 
        cat_crack.installed_costs = {'Tank':0}
    ...
    data = []
    for u in sys.flowsheet.stream:
        data.append([u.ID, u.get_total_flow("kg/hr")])
    df = pd.DataFrame(data, columns=['ID', 'Flow'])
    df.to_csv('output'+scenario["name"]+'_FCC.csv', index=False)
    return sys

system = run_scenario(scenarios[1])
#%%
# Check material balances
print("Feeds")
for f in sorted(sys.feeds, key=lambda x: x.F_mass, reverse=True):
    print(f"Feed: {f.ID}, {f.F_mass:.2f} kg/hr, {(f.F_mass - f.imass['water']):.2f} kg/hr")

print("\nProducts")
for p in sorted(sys.products, key=lambda x: x.F_mass, reverse=True):
    print(f"Product: {p.ID}, {p.F_mass:.2f} kg/hr, {(p.F_mass - p.imass['water']):.2f} kg/hr, {(p.F_mass - p.imass['water'])/sys.flowsheet.stream['HDPE_Feed'].F_mass:.2f} kg/kg feed")
#%%
#  %% 
# all_scen = []
...
for scen_label, scenario in enumerate(scenarios):
    ...
    for p in system.products:
        yields[scen_label][p.ID] = p.F_mass / sys.flowsheet.stream['HDPE_Feed'].F_mass
    
    for p in system.products:
        yields[scen_label][p.ID + "_no_water"] = (p.F_mass - p.imass["water"]) / sys.flowsheet.stream['HDPE_Feed'].F_mass

#%%  
df = pd.DataFrame(yields)
# column names are the scenario labels
df.columns = scenarios_labels
df.to_excel("Results/YieldsFCC.xlsx")
...
def run_GWP(syst,df_emi = EF):
    sys_labels = ["Feedstock Collection","Pretreatment & Pyrolysis","Product Fractionation", "FCC"]
    ...
    if scenario['Hydrocracking'] == "No":
        ...
        HC_feed = syst.flowsheet.unit.data["Cat_cracking"].ins[0].get_total_flow("bbl/hr")
        hydrogen_feed = syst.flowsheet.unit.data["Cat_cracking"].ins[1].get_total_flow("kg/hr")
        ...
        system_emissions["FCC"] = HC_diesel/850 * df_emi["Diesel"] + HC_electricity * df_emi["Electricity"] + hydrogen_feed * df_emi["Hydrogen"] #+ HC_cooling_water * df_emi["Water for cooling"]
...
groups = {"Pretreatment & Pyrolysis":['Handling','Mixer','Grinder',
        ...
        "Catalytic Cracking Unit": ['Compressor3','Hydrocracking','Cat_cracking_Unit','H2split'],
        "Others":[]
}
...
naphtha_label = ["CPY-FCC","TOD-FCC","HRT","HRT-FCC","Virgin"]
...
naphtha_list = [df_avoided_emission["Primary product emission per kg"]["CPY-FCC"],
                df_avoided_emission["Primary product emission per kg"]["TOD-FCC"],
                df_avoided_emission["Primary product emission per kg"]["HRT"],
                df_avoided_emission["Primary product emission per kg"]["HRT-FCC"],
                df_avoided_emission["Virgin product emission"]["CPY-FCC"]
                ]
...
for u in system.units:
    installed_costs.append([
        u.ID,
        u.installed_cost 
    ])
print(installed_costs)

df = pd.DataFrame(installed_costs, columns=["Equipment", "Cost (MM$)"])
# Save to csv:
df.to_csv("equipment_costs.csv", index=False)
