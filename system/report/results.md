...
5 → 64 → 128 → 64 → 8 architecture with batch normalisation and 10 %
dropout) on 566 literature pyrolysis experiments spanning polyethylene,
polypropylene, and polystyrene feedstocks at 300–800 °C and 0.5–120 s vapour
residence time<sup>[1](#ref1)</sup>.
...
inputs (HDPE, LDPE, PP weight fractions, temperature, and vapour residence
time).  Test-set parity plots (**Fig. 2**) confirm that PyrolysisNet captures
...
the underlying literature<sup>[2](#ref2)</sup>.

Linear reactor-type corrections, calibrated against published experimental
data for thermal oxodegradation<sup>[3](#ref3)</sup>, zeolite-catalysed
pyrolysis<sup>[1](#ref1)</sup>, and CO₂-plasma
pyrolysis<sup>[4](#ref4)</sup>, extend the base model to each upstream
technology (corrected_*i*(T) = base_*i* + α_*i* + β_*i* × T; see
Supplementary Tables S10–S12).  Temperature and composition sweeps
(**Fig. S3–S5**) reproduce physically expected trends: liquid yield rises
then plateaus above ~500 °C while gas yield increases monotonically, and
LDPE-rich feeds favour liquid over gas relative to HDPE-rich
...
pyrolysis-eligible fraction<sup>[6,](#ref6)[7,](#ref7)[8](#ref8)</sup>), the
four upstream pathways—thermal oxodegradation (TOD), conventional pyrolysis
(CP), catalytic pyrolysis (CPY), and plasma pyrolysis (PLASMA)—merge through
shared downstream distillation, hydrocracking (HC), and fluid catalytic
cracking (FCC) to produce naphtha, diesel, wax, ethylene, propylene, butene,
BTX, hydrogen, and oxygenated organics (alcohols, acids, carbonyls, olefins,
paraffins).
...
life<sup>[9](#ref9)</sup>.  Put differently, the plant can afford to *pay* for
waste-plastic intake rather than charge a tipping fee, an outcome that is
unique among published single-technology pyrolysis
assessments<sup>[10,](#ref10)[11](#ref11)</sup>.

The negative GWP (−0.315 kg CO₂-eq kg⁻¹ feed) reflects the displacement
methodology<sup>[12](#ref12)</sup>: product credits from displacing
fossil-derived naphtha (0.366 kg CO₂-eq kg⁻¹), diesel (0.475), ethylene
(1.396), propylene (1.435), and hydrogen (2.277) exceed the process burdens
from grid electricity, natural gas, and FCC off-gas
emissions<sup>[13](#ref13)</sup>.

**Figure 1.** Superstructure process flow diagram showing the four upstream
conversion technologies (TOD, CP, CPY, PLASMA), three splitters (x₁–x₃),
shared downstream separation (DISTILLATION), wax upgrading splitter (x₄),
and product streams.

![Superstructure PFD](../system_diagram_thorough.png)
...
over the four continuous split fractions using Nelder-Mead
simplex<sup>[14](#ref14)</sup> (50 iterations, x-tolerance 0.01, adaptive step
sizing).  The Pareto frontier for the baseline price scenario (**Fig. 3**)
reveals a tight cluster of near-optimal solutions rather than a broad
trade-off curve, indicating that both objectives can be improved simultaneously
by routing more feed through the PLASMA pathway.
...
Each point is one Nelder-Mead evaluation; the star marks the optimal solution.

![Pareto baseline](../pareto_scatter_baseline.png)
...
(Supplementary Table S19): *baseline* (all mid-range), *high_fuel* (fuels at
peak, others at trough), *high_chem* (chemicals at peak), and *high_organics*
(specialty organics at peak)<sup>[15,](#ref15)[16,](#ref16)[17](#ref17)</sup>.

**Figure 4.** Pareto frontiers for all four price scenarios, showing how the
MSP–GWP trade-off shifts with market conditions.

![Pareto by scenario](../pareto_scatter_by_scenario.png)
...
(−0.876 kg CO₂-eq kg⁻¹)—nearly three times the baseline—because oxygenated
PLASMA products (alcohols, acids, carbonyls) displace emission-intensive
conventional chemical production<sup>[4,](#ref4)[12](#ref12)</sup>.  It also
achieves a negative carbon abatement cost (−$0.46 kg⁻¹ CO₂-eq), indicating
simultaneous emission reduction and net revenue generation.
...
figure (**Fig. 6**) reveals the following hierarchy of decision-variable
influence:
...
1. **CPY vs. PLASMA allocation (x₃) is the dominant lever.**  Moving toward
   ...
   are favourable—a rare alignment of economic and environmental incentives.
...
3. **TOD vs. CP (x₂) is the least influential variable.**  Both thermal
   ...
   economics, consistent with prior single-technology comparisons<sup>[3,](#ref3)[18](#ref18)</sup>.
...
**Figure 6.** Sensitivity contour plots (6 pairwise combinations × 3
objectives: MSP, GWP, CAC).  Colour scales are independent for each subplot.

![Contour plots](../contours.png)

---

## 3.6 Comparison with Literature and Limitations

The superstructure framework offers significant advantages over single-pathway
designs.  Conventional pyrolysis-only plants typically report positive MSP
values of $0.05–0.30 kg⁻¹, requiring a tipping fee to achieve
profitability<sup>[10,](#ref10)[11,](#ref11)[18](#ref18)</sup>.  Our
...
range from approximately −0.1 to −0.3 kg CO₂-eq kg⁻¹
feed<sup>[19,](#ref19)[20](#ref20)</sup>.  The superstructure matches this
range at baseline (−0.315) and substantially exceeds it in the high_organics
scenario (−0.876), owing to the high displacement credits of oxygenated
chemicals produced by the PLASMA pathway.
...
regionally<sup>[6](#ref6)</sup>, which would shift both yields and economics.
...
<a id="ref19"></a>[19] Jeswani, H.K. *et al.* Life cycle environmental
...
<a id="ref20"></a>[20] Meys, R. *et al.* Achieving net-zero greenhouse gas
