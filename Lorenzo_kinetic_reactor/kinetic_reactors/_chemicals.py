...
chemical_groups = dict(
    ...
    InorganicSolubleSolids = ('AmmoniumSulfate', 'NaOH', 'KOH', 'HNO3', 'NaNO3',
    ...
    OtherOrganics = ('Xylitol', 'LacticAcid', 'SuccinicAcid', 'EthylLactate',
                     'EthylAcetate', 'EthylSuccinate', "Monoethanolamine"),
...
vle_chemicals = ('CH4', 'CO2', "Monoethanolamine",'Ethanol', 'H2O', 'EthylAcetate', 'AceticAcid', 'EthylLactate',
...
def create_chemicals(set_thermo=True):
    ...
    H2O.mu.l.add_model(1.002)

    ##### Gases #####
    add_chemical('O2', phase='g', Hf=0)
    add_chemical('N2', phase='g', Hf=0)
    add_chemical('H2', phase='g', Hf=0)
    add_chemical('CH4')#, phase='g') #ch4_rho_update
    add_chemical('CO', search_ID='CarbonMonoxide', phase='g', Hf=-26400*_cal2joule)
    CO2 = add_chemical('CO2')
    # CO2.Cn.g.add_method(39.8)
    # CO2.Cn.l.add_method(39.8)
    # CO2.V.l.add_method(4.7517017335354925e-05)
    # CO2.V.l.method = "USER_METHOD"
    ...
    ##### Soluble organics #####
    ethanol = add_chemical('Ethanol')
    ethanol.mu.l.add_model(1.2)
    add_chemical('AceticAcid')
    monoethanolamine = add_chemical('Monoethanolamine')#, phase = 's')
    monoethanolamine.mu.l.add_model(1.004)
    ...
    Extract= add_chemical('Extract', search_ID='GluconicAcid', phase='l')
    Extract.copy_models_from(Glucose)
    ...
    Z_mobilis = add_chemical('Z_mobilis', search_db=False, phase='l',
                               formula='CH1.8O0.5N0.2', Hf=-31169.39*_cal2joule)
    WWTsludge = add_chemical('WWTsludge', search_db=False, phase='s',
                             formula='CH1.64O0.39N0.23S0.0035', Hf=-23200.01*_cal2joule)
    Cellulase = add_chemical('Cellulase', search_db=False, phase='s',
                             formula='CH1.64O0.39N0.23S0.0035', Hf=-23200.01*_cal2joule)
    ...
    HMF = add_chemical('HMF', Hf=-99677*_cal2joule, Tb=291.5+273.15, Hfus=19800, phase='s')
    ...
    CoumaricAcid = add_chemical('CoumaricAcid', Glucan)
    FerulicAcid = add_chemical('FerulicAcid', Glucan)
    ...
    add_chemical("CaO", phase='s')
    add_chemical('Ash', search_ID='CaO', phase='s', Hf=-151688*_cal2joule, HHV=0, LHV=0)
    add_chemical('Biochar', search_ID='CaO', phase='s', Hf=-151688*_cal2joule, HHV=0, LHV=0)
    ...
    #### Other
    add_chemical('S') #phase = 's')
    glycerol = add_chemical('Glycerol',
                  ...
                  Hf=-975708.8)
    dap = add_chemical('DAP', search_ID='DiammoniumPhosphate', phase='l')
    
    Dinitrogen = add_chemical('Dinitrogen', search_ID='N2', phase='g')
    Alumina = add_chemical('Alumina', search_ID='Al2O3', phase='s')
    Ar= add_chemical('Ar', search_ID='Argon', phase='g')
    Ni = add_chemical('Ni', phase='s')
    Fe = add_chemical('Fe', phase='s')
    C = add_chemical('C', phase='s')
    C_amorphous = add_chemical('C_amorphous', search_db=False, phase='s', formula='C', Hf=0)# Add amorphous carbon

    # To add slaughterhouse waste, which is not in the database:
    SW = add_chemical('SW', search_db=False,  phase='l')
    SW.copy_models_from(Glucose)

    # To add domestic waste, which is not in the database:
    DW = add_chemical('DW',  search_db=False, phase='l')
    DW.copy_models_from(Glucose)
    X1= add_chemical('X1',  search_db=False, phase='l')
    X1.copy_models_from(Glucose)
    X2= add_chemical('X2',  search_db=False, phase='l')
    X2.copy_models_from(Glucose)
    # To add Voltaic fatty acids, which are not in the database:
    VFA = add_chemical('VFA', search_db=False, phase='l')
    VFA.copy_models_from(Glucose)
...
#     ["H2", 'H2', None],
#     ["H2S", 'H2S', None],
#     ['WWTsludge', '110-59-8', 's'],
#     ['Cellulose', 'C6H10O5', 's'],
#     ['Xylan', 'C6H10O5', 's'],
#     ['CellulaseNutrients', 'C6H10O5', 's'],
#     ['Mannan', 'Mannan', 's'],
#     ['Lignin', 'Lignin', 's'],
#     ['Galactan', 'C6H10O5', 's'],
#     ['Glucan', 'Glucan', 's'],
...
#     ["Furfural", "Furfural", 's'],
#     ["Glycerol", "Glycerol", 's'],
#     ["LacticAcid", "LacticAcid", 's'],
#     ["SuccinicAcid", "SuccinicAcid", 's'],
#     ["HNO", "HNO", 'g'],
#     ["Denaturant", "Denaturant", 's'],
#     ["DAP", "DAP", 's'],
#     ["AmmoniumAcetate", "AmmoniumAcetate", 'l'],
#     ["AmmoniumSulfate", "AmmoniumSulfate", 'l'],
#     ["NaNO", "NaNO", 'l'],
#     ["Oil", "Oil", 'l'],
#     ["HMF", "HMF", 'l'],
#     ["NH", "NH", 'l'],
...
#     ["Sucrose", "Sucrose", 's'],
#     ["Mannose", "Mannose", 's'],
#     ["Galactose", "Galactose", 's'],
#     ["Arabinose", "Arabinose", 's'],
#     ["Extract", "Extract", 's'],
#     ["Tar", "Tar", 's'],
#     ["CaO", "CaO", 's'],
#     ["Ash", "Ash", 's'],
#     ["NaOH", "NaOH", 's'],
#     ["Lignin", "Lignin", 's'],
#     ["SolubleLignin", "SolubleLignin", 's'],
#     ["GlucoseOligomer", "GlucoseOligomer", 's'],
#     ["GalactoseOligomer", "GalactoseOligomer", 's'],
#     ["MannoseOligomer", "MannoseOligomer", 's'],
#     ["XyloseOligomer", "XyloseOligomer", 's'],
#     ["ArabinoseOligomer", "ArabinoseOligomer", 's'],
#     ["Z_mobilis", "Z_mobilis", 's'],
#     ["T_reesei", "T_reesei", 's'],
#     ["Cellulose", "Cellulose", 's'],
#     ["Protein", "Protein", 's'],
#     ["Enzyme", "Enzyme", 's'],
#     ["Xylan", "Xylan", 's'],
#     ["Xylitol", "Xylitol", 's'],
#     ["Cellobiose", "Cellobiose", 's'],
#     ["DenaturedEnzyme", "DenaturedEnzyme", 's'],
#     ["Arabinan", "Arabinan", 's'],
#     ["Mannan", "Mannan", 's'],
#     ["Galactan", "Galactan", 's'],
#     ["WWTsludge", "WWTsludge", 's'],
#     ["Cellulase", "Cellulase", 's'],
#     ["CoumaricAcid", "CoumaricAcid", 's'],
#     ["FerulicAcid", "FerulicAcid", 's']

# ]

# def define_chemicals(name, formula, phase):
#     try:
#         chemical = bst.Chemical(name, search_ID=formula, default=True, phase=phase)
#     except:
#         print(f"Chemical Error: {name} {formula} {phase}")
#         chemical = bst.Chemical.blank(name, phase=phase, Hf=-975708.8) # What does this represent?
#     chemical.default()
#     return chemical
# %%